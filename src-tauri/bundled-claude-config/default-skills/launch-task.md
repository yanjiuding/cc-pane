...
当目标项目路径为 WSL UNC 格式（如 `\\wsl.localhost\Ubuntu\home\user\repo`）时，`launch_task` 会自动检测并以 WSL 模式启动。

如果工作空间的 `defaultEnvironment` 为 `wsl`，即使项目路径是 Windows 本地路径，也会自动转换为 WSL 远端路径启动。用户明确要求在 Windows 本机启动时，传 `runtimeKind: "local"`。
